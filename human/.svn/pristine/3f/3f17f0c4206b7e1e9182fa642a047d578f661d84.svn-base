...
function initTable() {
	...
	$("#ccrTable").bootstrapTable({
		...
		columns : [ {
			...
			align : 'center',
			formatter : function(value, row, index) {
					return "<a onclick='edit(\"" + row.code + "\",\"" + row.deptName + "\")'>"+value+"</a>";
		}
		},{
			field : 'gender',
			title : '性别',
			...
			formatter : function(value, row, index) {
				var re="";	
					...
					re +="<button  class='layui-btn layui-btn-mini' style='margin-right:10px;' onclick='return comments(\"" 
						+ row.code + "\",\"" + row.name + "\");'><i class='fa fa-pencil-square-o'></i>&nbsp;评语填写</button>";
				return re;
			}
		}],
...
//编辑成绩
function edit(code,deptName){
	 var url = jsBasePath+"/Examinee/List/lookDetail.html?code="+code+"&deptName="+deptName;
	 layer.open({
	     type: 2,
	     shade : [ 0.5, '#000' ],
	     title: "编辑数据", //
