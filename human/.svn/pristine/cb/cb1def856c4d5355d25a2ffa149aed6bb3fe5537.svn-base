layui.use(['form','laydate'], function(){
  ...
  form.on('submit(demo1)', function(data){
	  var total=0;
		...
		/*if(!checkNumber(paperMonth)||paperMonth==""){
		...
		if(totalQNum!=null&&totalQNum!=""){
			if(total!=parseInt(totalQNum)){
				layer.alert("总题数与各知识点数总和不相等",{icon:2});
				return false;
			}
		}
		if(paperTime==""||parseInt(paperTime)<=0){
			layer.alert("答题时间需要大于0",{icon:2});
			return false;
		}
		if(totalQNum==""||parseInt(totalQNum)<=0){
			layer.alert("总题数需要大于0",{icon:2});
			return false;
		}
		var all={};
		var alldetails = [];
		...
		$(".score").each(function(){
			var level={};
			level.dicId=$(this).find("#score").attr("name");
			level.num=$(this).find("#score").val();
			var index=$(this).find("#score").attr("index");
			num=$(this).find("#score").val();
			if(level.num!=null){
		...
		if(scoreNum==null||scoreNum==""){
			layer.alert("优秀、良好、及格必填一项",{icon:2});
			flag=false;
			return false;
		}
		$(".config-paper").each(function(){
			if(flag){
				var layerHtml=$(this).find(".layer").html();
				if(layerHtml==null||layerHtml==""){
					var falseName= $(this).find(".info-kl").find(".knowledgeName").text();
					var zNum=$(this).find(".info-kl").find("span").find(".zsd").val();
					if(zNum!=''){
						layer.alert(falseName+"未配置",{icon:2});
						$(this).find(".info-kl").find("span").find(".zsd").focus();
						flag=false;
					}
				}
				var zsd = $(this).find('.zsd').val();
				var zsdName=$(this).find('.knowledgeName').text();
				var knowledgeId=$(this).find('.knowledgeName').attr("index");
				$(this).find(".condiffi").each(function(){
					var detail ={};
		...
		var str =JSON.stringify(all);
			...
			$.post(jsBasePath+"/jzbTest/jpConfig/save.html",{"str":str},function(data,status){
				layer.close(index); 
				if(data.flag==false){
  ...
  $(".month").click(function(){
	...
	$.post(jsBasePath+"/jzbTest/jpConfig/month.html",{"id":$("#id").val(),'monthArray':monthArray,"subject":subject,"classType":classType,"grade":grade},function(data,status){
		var html="<label class='layui-form-label'>知识点：</label><div class='layui-input-block'>";
				$.each(data, function(i, info) {
					if(info.titleNum==""){
						info.titleNum=0;
					}
	  html+="<div class='config-paper'><p class='info-kl' index='"+info.id+"' style='margin:1%;' >" +
	  		"<span class='knowledgeName' style='' index='"+info.id+"'>"+info.knowledge+"</span> <span class='qMonth' hidden='true'>"+info.qMonth+"</span>" +
	  				"<span class='maxNum' index='"+info.count+"'>("+info.count+")</span>" +
	  				...
	  				"<button  class='layui-btn layui-btn-mini a config' style='margin-left:6%;' onclick='config(this)'>配置</button></p>" +
	  				"<div class='layer'>" +
	  				"</div></span></div>" ;
				});
	  $(".knowledge").html(html);
...
function config(obj){
	...
	if(parseInt(titleNum)>1){
			$.post(jsBasePath+"/jzbTest/jpConfig/queryByKnow.html",{"knowledgeId":knowledgeId,"classType":classType,"grade":grade,"subject":subject,"month":month,"qMonth":monthArray,"type":"add"},function(data,status){
				var html="";
				$.each(data,function(i,info){
					html+="<p ><span class='condiffi' ><span class='diflevel' value='"+info.COUNT+"' index='"+info.Q_DIFFICULTY+"'>难度"+info.Q_DIFFICULTY+"("+info.COUNT+"):</span>" +
							"<input class='difficultnum'  type='text'  />" +
							"<span class='title_num' hidden='true'>"+info.title_num+"</span></span></p> "
				});
			...
			$.post(jsBasePath+"/jzbTest/jpConfig/queryByKnow.html",{"knowledgeId":knowledgeId,"classType":classType,"grade":grade,"subject":subject,"month":month,"qMonth":monthArray,"type":"add"},function(data,status){
				var html="";
				$.each(data,function(i,info){
					html+="<p ><span class='condiffi' ><span class='diflevel' value='"+info.COUNT+"' index='"+info.Q_DIFFICULTY+"'>难度"+info.Q_DIFFICULTY+"("+info.COUNT+"):</span><input class='difficultnum'  type='text'  /></span>" +
							"<span class='title_num' hidden='true'>"+info.title_num+"</span> </p>" 
							 ;
				});
				html+="<p > <button class='layui-btn' style='width: 99px;margin-left: 76px;margin-top: 11px;' onclick='sumbitdif(this)'>确定</button><button class='layui-btn' style='width: 99px;margin-left: 76px;margin-top: 11px;' onclick='cancle(this)'>取消</button></p>"
					$(obj).parent().parent().find(".layer").html(html);
...
function toaddexam(){
	 var month=$("#month").val();
	 var id=$("#id").val();
	 var url = jsBasePath+"/jzbTest/jpConfig/toAddExam.html?month="+month+"&id="+id;
	 layer.open({
	     type: 2,
	     shade : [ 0.5, '#000' ],
	     title: "配置考试", //
...
function sumbitdif(obj){
	...
	$(obj).parent().parent().find(".difficultnum").each(function(){
		    ...
		    var titleNum =$(this).parent().parent().find("span[class=title_num]").text();
...
$("#view_phone").click(function(){
	...
	var qrUrl=$("#qrUrl").val();
	var url = qrUrl+"/jzbTest/jpConfig/view_phone.html?"+
