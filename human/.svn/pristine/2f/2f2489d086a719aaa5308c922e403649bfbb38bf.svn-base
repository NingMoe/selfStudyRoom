			...
			if (data.index == 0) {
				...
				f2.load({
					elem : '#waitDiv', //指定列表容器
						...
						$.post(jsBasePath + '/wxCustomer/customerMailFox/getMailAllList.html', {
							pageNow : page,pageSize:5,queryType:1
						}, function(res, status) {
							layer.close(index);
							$.each(res.records, function(i, item) {
								html='<div class="list1">'+
									    '<h3 class="clearfix"><span>家长原始描述:</span> <b>'+new Date(item.createTime).Format('MM-dd HH:mm')+'</b></h3>'+
										'  <p>'+ item.desc +'</p>'+
			...
			if (data.index == 1) {
					...
					f2.load({
						elem : '#operingDiv', //指定列表容器
							...
							var lis = [];
							//以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
							$.post(jsBasePath + '/wxCustomer/customerMailFox/getMailMyList.html', {
								...
								$.each(res.records, function(i, item) {
									var html='<div class="list2">'+
	  								...
	  								'    </li>'+
	  								'    <li>'+
	  								'      <h4 class="clearfix"><span>'+new Date(item.createTime).Format('MM-dd HH:mm')+'</span><b onclick="showPopup('+item.id+')">【查看详情】</b></h4>'+
	  								'    </li>  '+
	  								'  </ul>'+
	  								'</div>';
									lis.push(html);
