layui.use(['form','laydate'], function(){
    ...
    form.on('submit(demo1)', function(data){
  	  ...
  	  if(!flag){
  		...
  		$.ajax({
			url : jsBasePath+"/basic/activity/edit.html",
...
//校验商品数目
function checkNumber(productId,total){
	if(total==""){
		layer.alert("商品数目必填!",{icon:2});
		...
		$.ajax({
			url : jsBasePath+"/basic/activity/checkProductNumber.html",
			data : {productId:productId,newTotal:total},
			async:false,
			dataType : "json",
			type : "post",
			success:function(res){			
