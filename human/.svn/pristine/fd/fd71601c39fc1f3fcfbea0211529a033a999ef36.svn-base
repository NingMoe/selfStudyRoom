...
function initTable() {
	//初始化Table 不 
	$('#resumeKeywordTable').bootstrapTable('destroy');
	$("#resumeKeywordTable").bootstrapTable({
		...
		columns : [ {
			...
			formatter : function(value, row, index) {
				var page = $("#resumeKeywordTable").bootstrapTable("getPage");
...
function add(){
	layer.open({
		 ...
		 offset : [ '10%' ],
		 area: ['550px','400px'],
		 content:jsBasePath + '/basic/resumeKeyword/toAdd.html', 
...
//删除
function del(id){
	layer.confirm("确定删除该简历解析关键词?删除后不可恢复!", {
		  btn: ['是','否'] ,//按钮
		  offset: '10%',
		  btnAlign:'c'
		}, function(index){
			$.post(jsBasePath+"/basic/resumeKeyword/delete.html",{deleteIds:id},function(data,status){
				layer.close(index);
				if(data!=null){
					layer.msg(data.message);
					if(data.flag==true){
					   $("#resumeKeywordTable").bootstrapTable('refresh');
					}
				}
			},"json");
		}, function(index){
...
//批量删除
function batch_del(){
	var selectIds=getSelectId("resumeKeywordTable");
	if(selectIds==""){
