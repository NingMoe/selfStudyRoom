//注意：折叠面板 依赖 element 模块，否则无法进行功能性操作
  layui.use(['form', 'layedit', 'laydate','upload','element'], function(){
		...
		if (data.index == 1) {
			...
			$.post(jsBasePath + '/free/resume/gtRecord.html', {resumeId:$("input[name='id']").val()}, function(data, status) {
				layer.close(index);
				if (!data.flag) {
					$("#gtNR").html("沟通记录获取失败");
				}else {
...
*/    		});
  	  ...
  	  var s = layui.upload({
  		  elem:"#file1",
  		  url: jsBasePath+'/free/resume/editResumeHead.html',
  		  isAuto:true,
	  ...
	  var x = layui.upload({
  		  elem:"#file2",
  		  url: jsBasePath+'/free/resume/uploadResumePhoto.html',
  		  isAuto:true,
  		  before: function(input){
  			indexFile1=layer.load();
  		  },
    	  ...
    	  form.on('submit(editApp)', function(data){
  			...
  			$.post(jsBasePath+'/free/resume/updateResumeBasic.html',{jstr:JSON.stringify(getData())},function(data,status){
  				layer.close(index); 
  			  if(!data.flag){
  ...
  /**
	...
	function delResumePhoto(photoId,path){
		  var index = layer.load(3, {shade: [0.3]});
			$.post(jsBasePath+'/free/resume/delResumePhoto.html',{photoId:photoId,path:path},function(data,status){
				layer.close(index); 
			  if(!data.flag){
				  layer.alert(data.msg,{icon:2});
			  }else{
 ...
 function dowEnclosure(){
	 var url=jsBasePath+"/free/resume/dowEnclosure.html?id="+$("#apId").val();
	   window.location.href = url;    
 }
...
function feedback(){
	var url = jsBasePath + "/free/resume/feedback.html?flowCode="+$("#flowCode").val()+"&userName="+$("#userName").val();
	layer.open({
		type : 2,
		shade : [ 0.5, '#000' ],
		area : [ '50%', '550px' ],
...
function toSendMsgNoLogin(tel,seekerId,resumeId){
	var url = jsBasePath + "/free/resume/sendView.html?sendTel="+tel+"&seekerId="+seekerId+"&resumeId="+resumeId+"&userName="+$("#userName").val();
	layer.open({
		type : 2,
		shade : [ 0.5, '#000' ],
		offset : [ '5%' ],
