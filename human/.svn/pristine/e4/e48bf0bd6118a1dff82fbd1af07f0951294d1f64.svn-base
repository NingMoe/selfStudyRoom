...
$(function() {
	...
	matchclass_type = getQueryString("matchclass_type");
...
function initTable() {
	...
	$("#processDefTable").bootstrapTable({
		url : jsBasePath + '/ielts/teachermatchclass/query.html?teacher_id=' + teacher_id + '&matchclass_type=' + matchclass_type, // 请求后台的URL（*）
		// method: 'get', //请求方式（*）
		method : 'post',
		...
		columns :
			[
			 ...
			 {
				 field : 'matchclass_type',
				 title : '赛课等级',
				 align : 'center',
				 formatter : function(value, row, index) {
					 var i = "";
					 if(row.matchclass_type == 1){
						 i = "集团赛课";
					 }
					 
					 if(row.matchclass_type == 2){
						 i = "区域赛课";
					 }
					 
					 if(row.matchclass_type == 3){
						 i = "部门赛课";
					 }
					 return i;
				 }
			 ...
			 {
				 field : 'matchclass_grade',
				 title : '获奖名次',
				 align : 'center',
			     editable: {
			         type: 'select',
			         title: '名次',
			         source:[{value:"1",text:"一等奖"},{value:"2",text:"二等奖"},{value:"3",text:"三等奖"}]
			     }
			 },
			 {
				 field : 'create_time',
			 ...
			 onEditableSave: function (field, row, oldValue, $el) {
	                $.ajax({
	                    type: "post",
	                    url: jsBasePath + "/ielts/teachermatchclass/updateteachermatchclass.html",
	                    data: row,
	                    dataType: 'JSON',
...
function deletesteachertktinfo() {
	...
	layer.confirm("确认删除选中的行？", {
		  ...
		  btnAlign:'c'
			...
			layer.close(index);
			$.ajax({
				url : jsBasePath + "/ielts/teachermatchclass/deletesteachermatchclass.html",
				type : "POST",
				dataType : "json",
				data : {
					ids : ids,
