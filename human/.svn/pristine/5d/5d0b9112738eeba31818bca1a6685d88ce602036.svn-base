...
<body>
	<div class="container" style="margin-top: 10px;">
			...
			<input type="hidden" value="${fbb.id}" id="baseId">
			<input id="userName" type="hidden" value="${fbb.userName}">
			<input type="hidden" value="${fbb.closeType}" id="closeType">
			<input type="hidden" value="" id="score">
			<c:if test="${fbb.closeType==0}">
			...
			<div class="form-group">
				...
				<div class="col-sm-10"   >
				 <textarea class="layui-textarea" rows="3"   <c:if test="${fbb.closeType==1}"> lay-verify="required"</c:if>  id="closeDesc"></textarea>
				</div>
			</div>
	...
	layui.use([ 'element', 'form' ], function() {
		...
		$.post(jsBasePath + '/free/feedBack/closeFeedBack.html', {id:$("#baseId").val(),score: $("#score").val(),closeDesc:$("#closeDesc").val(),closeType:$("#closeType").val(),userName:$("#userName").val()}, function(data, status) {
			layer.close(index);
			if(data.flag){
