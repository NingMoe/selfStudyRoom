...
(function(root, factory) {
	...
	function photoClip(container, option) {
				...
				return false;
				 ...
				 EXIF.getData(file, function() {  
			       ...
			       /*     alert(Orientation);*/
					...
					fileReader.onload = function(e) {
						...
						if (kbs > 1024) {
							...
							$tempImg.load(function(e) {
								...
								// 将图片进行压缩
								var newDataURL = compressImg(this, quality, angleOffset, outputType,Orientation);
								createImg(newDataURL);
							});
						/*	console.log("suolue==========="+this.result);*/
							$tempImg.attr("src", this.result);
						} else {
						/*	console.log("busuolue==========="+this.result);*/
					...
					loadStart.call(fileReader, file);
			        });
			}
		});
		...
		function compressImg(sourceImgObj, quality, angleOffset, outputFormat,Orientation){
			quality = quality || .8;
				...
				if (Orientation != "" && Orientation != 1) {
					switch (Orientation) {
						case 6: //需要顺时针（向左）90度旋转  
							...
							rotateImg(sourceImgObj, cvs,1);
							break;
						case 8: //需要逆时针（向右）90度旋转  
							// alert('需要顺时针（向右）90度旋转');  
							rotateImg(sourceImgObj, cvs,3);
							break;
						case 3: //需要180度旋转  
							// alert('需要180度旋转');  
							rotateImg(sourceImgObj, cvs,2); //转两次  
