		...
		<c:forEach var="detail" items="${cm.cmr}">
			<div >
				...
				<div>${detail.desc}</div>
					...
					<div class="time" style="margin-top: 3px;text-align:right;">${cm. solUser}回复于<fmt:formatDate value='${detail.operTime}' pattern='yyyy-MM-dd HH:mm:ss'/></div>
						</c:if>
				<c:if test="${detail.type!=1}">
...
<div class="form-group" style="text-align: center; "><button  lay-submit="" lay-filter="sub1"  type="button" class="layui-btn layui-btn-primary">回复/补充问题</button></form></div>
		</c:if>
	<c:if test="${cm.state==5}">
		<div class="form-group" style="text-align: center; margin-top: 20px;">
		<c:if test="${empty cm.score}">
		<button type="button" class="layui-btn" onclick="subScore(${cm.id});">&nbsp;评分</button>&nbsp;&nbsp;
		</c:if>
...
function showImgs(obj,k){
	...
	function subScore(baseId) {
		layer.open({
			  title:"评分",
			  type: 2,
			  area: ['90%', '60%'], //宽高
			  content:  jsBasePath+'/wxCustomer/mailFox/subScoreView.html?id='+baseId
			});
	}
	...
	function jxfeedback(){
		...
		for(var j=0;j<imgs.length;j++){
	      ...
	      xhr.open('post', jsBasePath+'/wxCustomer/mailFox/addMailRecord.html');
