...
<body >
	...
	<form class="layui-form"  id="editForm" action="" method="post">
		...
		面试记录<button type="button"  onclick="showQrCode('<%=basePath%>/free/resume/quickUploadCustomPhoto.html?flowCode=${flowCode}&nodeId=${nowNode}&approver=${localUser}','面试反馈图片上传')"
	    	  ...
	    	  ,layer = layui.layer;
				  ...
				  $(".imgs img").load(function(){initImgWith();});
						form.on('submit(sub)', function(data,event){
				    		 var index = layer.load(3, {shade: [0.3]});
				  			$.post(jsBasePath+'/recruit/resumeManager/feedbackAdd.html',{jstr:JSON.stringify($('#editForm').serializeJson())},function(data,status){
				  				layer.close(index); 
				  			  if(!data.flag){
				  				  layer.alert(data.msg,{icon:2});
				  			  }else{
