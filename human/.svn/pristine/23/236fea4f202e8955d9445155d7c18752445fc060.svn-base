...
$(function(){
	$(".add").click(function() {
		...
		var price=parseInt($("#privce").val());
		var totalMoney=parseInt((buySum+1)*price);
		$("#buySum").val(buySum+1);
		$(".prc_input").val(totalMoney);
	});
...
function confirmBuy(){
	...
	if(ua.match(/MicroMessenger/i) != 'micromessenger'){//不是，采用浏览器H5支付
		payByH5(activityId,productId,name,buySum,telephone,code,text1,index);
	}else{//是，采用公众号支付
		payByPublic(activityId,productId,name,buySum,telephone,code,text1,index);
	}
}
//H5支付
function payByH5(activityId,productId,name,buySum,telephone,code,text1,index){
	$.ajax({
		type : "POST",
		dataType : "JSON",
		url : jsBasePath+"/activity/orderInfo/payOrderInfoByH5.html",
		data : {
...
//公众号支付
function payByPublic(activityId,productId,name,buySum,telephone,code,text1,index){
	$.ajax({
		type : "POST",
		dataType : "JSON",
		...
		success : function(data){
			if(data.flag){
				window.location.href = jsBasePath+"/activity/orderInfo/insertOrderInfo.html?activityId="+activityId+"&productId="+productId+"&name="+name+"&telephone="+telephone+"&buySum="+buySum+"&text1="+text1;	
			}else{
				layer.close(index);
				alert(data.message);
			}
