...
function initTable() {
	...
	$("#processDefTable").bootstrapTable({
		url : jsBasePath + '/ielts/teacherstudent/query.html', // 请求后台的URL（*）
		// method: 'get', //请求方式（*）
		method : 'post',
			 ...
			 },
			 {
				 ...
				 formatter : function(value, row, index) {
					 return "<sec:authorize ifAnyGranted='ROLE_act_isagree'>&nbsp;</sec:authorize>" +					 	
					 	"&nbsp;<button  class=\"layui-btn layui-btn-mini\" onclick=\"return studentenrollchange(\'" + row.id + "\',\'"+row.student_name+"\',\'"+row.student_phone+"\');\">" +
...
function studentenrollchange(id,student_name,student_phone) {
	layer.open({
		...
		offset : [ '10%' ],
		area : [ '800px', '90%' ],
		content : jsBasePath + '/ielts/teacherstudent/enrollinfoview.html?id=' + id + "&student_name=" + student_name + "&student_phone=" +student_phone,
		end : function() {
