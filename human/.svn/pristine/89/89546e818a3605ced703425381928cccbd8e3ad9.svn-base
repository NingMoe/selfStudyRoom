...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload:function(opt,serverCallBack) {
			...
			webUploader.options.formData =opt.formData;
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {			
			var div='<div style="width:300px;margin:10px 15px;">'+
						 '<input type="checkbox" id="fastFlag" name="fastFlag" value="0" >'+
					     '将该组图片简历设为通用快速投递简历'+
				     '</div>';
			 div += '<div class="diyButton"> \
						<a class="diyStart" style="margin-right:15px;" href="javascript:void(0)">开始上传</a> \
						<a class="diyCancelAll" style="margin-right:15px;" href="javascript:void(0)">全部取消</a> \
			        </div>';
