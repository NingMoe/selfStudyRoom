			...
			<div class="layui-inline">
				...
				<div class="layui-input-inline" style="width: 150px;">
					<input type="text" id="deptName"
						autocomplete="off" class="layui-input" placeholder="部门名称模糊搜索">
				</div>
				</div>
			<div class="layui-inline">
    ...
    <script type="text/javascript">
    	function initTable() {
    		...
    		$("#roleTable").bootstrapTable({
    			url : jsBasePath + '/customer/centerDept/query.html', //请求后台的URL（*）
    			//method: 'get',      //请求方式（*）
    			method : 'post',
    	 ...
    	 */
    	function add(){
    		 var url = jsBasePath+"/customer/centerDept/addView.html";
    		  layer.open({
    	      type: 2,
    	      shade : [ 0.5, '#000' ],
    			offset : [ '10%' ],
    	 ...
    	 */
    	function edit(id){
    		 var url = jsBasePath+"/customer/centerDept/toEdit.html?id="+id;
    		  layer.open({
    	       type: 2,
    	       shade : [ 0.5, '#000' ],
    			offset : [ '10%' ],
    	...
    	function del(id){
    		layer.confirm("确定要删除该部门？", {
    			  ...
    			  btnAlign:'c'
    				...
    				$.post(jsBasePath+"/customer/centerDept/del.html",{deleteIds:id},function(data,status){
    					layer.close(index);
    					if(data!=null){
    						layer.msg(data.msg);
    						if(data.flag==true){
    	...
    	function bath_del(){
    		...
    		layer.confirm("确认要删除所选部门?", {
    			  btn: ['是','否'] ,//按钮
    			  offset: '10%',
    			  btnAlign:'c'
    			}, function(index){
    				$.post(jsBasePath+"/customer/centerDept/del.html",{deleteIds:ids},function(data,status){
    					layer.close(index);
    					if(data!=null){
