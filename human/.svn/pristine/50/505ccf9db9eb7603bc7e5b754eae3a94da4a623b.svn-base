...
;(function(b) {
	var a = {
		init : function(c) {
			return this.each(function() {
				...
				this.score = a._createScore.call(this);a._apply.call(this, this.opt.score);
				...
				if (this.opt.readOnly) {
					...
					e.css("cursor", "pointer");a._binds.call(this);
				}
				if (this.opt.width !== false) {
					e.css("width", d);
				}
				a._target.call(this, this.opt.score);e.data({
					settings : this.opt,
					raty : true
				});
			});
		...
		_apply : function(c) {
			if (c && c > 0) {
				c = a._between(c, 0, this.opt.number);this.score.val(c);
			}
			a._fill.call(this, c);
			if (c) {
		...
		_binds : function() {
			...
			a._bindClick.call(this);a._bindOut.call(this);a._bindOver.call(this);
		},
		...
		_bindOut : function() {
			...
			b(this).on("mouseleave.raty", function(d) {
				...
				a._apply.call(c, e);a._target.call(c, e, d);
				if (c.opt.mouseout) {
		...
		_bindOverCancel : function() {
			...
			c.cancel.on("mouseover.raty", function(d) {
				b(this).attr("src", c.opt.path + c.opt.cancelOn);c.stars.attr("src", c.opt.path + c.opt.starOff);a._target.call(c, null, d);
				if (c.opt.mouseover) {
			...
			c.stars.on(e, function(g) {
				...
				if (c.opt.half) {
					var f = parseFloat((g.pageX - b(this).offset().left) / c.opt.size),
						j = (f > 0.5) ? 1 : 0.5;
					h = h - 1 + j;a._fill.call(c, h);
					if (c.opt.precision) {
						h = h - j + f;
					}
					a._roundStars.call(c, h);d.data("score", h);
				}
				else {
					a._fill.call(c, h);
				}
		...
		_lock : function() {
			...
			b(this).data("readonly", true).css("cursor", "").attr("title", c);this.score.attr("readonly", "readonly");this.stars.attr("title", c);
			if (this.cancel) {
				this.cancel.hide();
			}
		},
		...
		_unlock : function() {
			b(this).data("readonly", false).css("cursor", "pointer").removeAttr("title");this.score.removeAttr("readonly", "readonly");
		...
		cancel : function(c) {
			return this.each(function() {
				if (b(this).data("readonly") !== true) {
					a[c ? "click" : "score"].call(this, null);this.score.removeAttr("value");
				}
			});
		},
		click : function(c) {
		...
		readOnly : function(c) {
			return this.each(function() {
				...
				if (d.data("readonly") !== c) {
					if (c) {
						d.off(".raty").children("img").off(".raty");a._lock.call(this);
					}
					else {
		...
		setScore : function(c) {
			return b(this).each(function() {
				if (b(this).data("readonly") !== true) {
					a._apply.call(this, c);a._target.call(this, c);
				}
			});
		}
	};
	...
	b.fn.raty.defaults = {
		...
		cancelOff : jsBasePath+"/static/raty/lib/img/cancel-off.png",
		cancelOn :  jsBasePath+"/static/raty/lib/img/cancel-on.png",
		cancelPlace : "left",
		click : undefined,
		half : false,
		halfShow : true,
		...
		size : 16,
		space : true,
		starHalf : jsBasePath+"/static/raty/lib/img/star-half.png",
		starOff : jsBasePath+"/static/raty/lib/img/star-off.png",
		starOn : jsBasePath+"/static/raty/lib/img/star-on.png",
		target : undefined,
		targetFormat : "{score}",
		targetKeep : false,
		targetText : "",
