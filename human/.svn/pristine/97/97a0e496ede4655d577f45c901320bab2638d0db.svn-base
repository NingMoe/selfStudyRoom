		...
		<input type="hidden" name="id" value="${dm.id}">
			...
			<div class="layui-inline " style="margin: 0 auto 10px auto">
				...
				<div class="layui-input-block">
					<select id="used" name="used" lay-filter="aihao">
						<option value="是" <c:if test="${'是'  eq dm.used }">selected="selected"</c:if>>是</option>
		    			<option value="否" <c:if test="${'否'  eq dm.used }">selected="selected"</c:if>>否</option>
					</select>
				</div>
			</div>
			<div class="layui-form-item">
			...
			<div class="layui-form-item">
				<label class="layui-form-label">申请时间:<font color="red">*</font></label>
					<div class="layui-input-inline">
						<input class="layui-input" placeholder="申请时间"  readonly="readonly" id="applyDate"  lay-verify="required" name="applyDate" value="${dm.applyDate}">
					</div>
			</div>
			<div class="layui-form-item">
   			    	<div class="layui-inline" style="margin: 0 auto 10px auto; width: 100%; margin-left: -2%!important;">
						...
						<div class="layui-input-block" style="width: 80%" id="class_type_checkbox">
							...
							<input name="sendReason"  type="radio"  value="超时"  title="超时" lay-filter="sendReason">
...
<script type="text/javascript"
	src="<%=basePath%>/static/datamanger/edit.js"></script>
</html>