		...
		<div class="layui-form-item collapse in" id="collapseOne">
			<div class="layui-form-item">
				...
				<div class="layui-input-inline" >
					...
					<option value="${dname }">${d.name}</option>
					</c:forEach>
					</select>
				</div>
				</div>
				...
				<div class="layui-input-inline" >
					<select id="state"   >
					<option value="">请选择</option>
					<option value="0" selected="selected">待完善</option>
					<option value="1">待处理</option>
					<option value="2">跟进中</option>
					<option value="3">已跟进</option>
...
function initTable(){
	 var columns = [{
			...
			formatter : function(value, row, index) {
					if(value==0){
						return "<font color=\"#ff0000\">待完善</font>";
					}
					if(value==1){
						return "<font color=\"#ff9933\">待处理</font>";
					}
					if(value==2){
						return "<font color=\"#ff9933\">待解决</font>";
					}
					if(value==3){
						return "<font color=\"#ff9933\">跟进中</font>";
					}
					if(value==4){
						return "<font color=\"#ff9933\">已跟进</font>";
					}
					if(value==5){
						return "<font color=\"#00FF33\">已处理</font>";
					}
			}
		}];
		 columns.push({
			...
			formatter : function(value, row, index) {
				var op = "";
				if(row.state==0){
					op += "<button  class='layui-btn layui-btn-mini layui-btn-danger' onclick='return edit(\"" + row.id + "\");'><i class='fa fa-pencil-square'></i>&nbsp;完善信息</button >&nbsp;";
				}else if(row.state==5){
					op += "<button  class='layui-btn layui-btn-mini' onclick='return showDetail(\"" + row.id + "\");'><i class='fa fa-search'></i>&nbsp;查看详情</button >&nbsp;";
				}else{
					op += "<button  class='layui-btn layui-btn-mini layui-btn-normal' onclick='return edit(\"" + row.id + "\");'><i class='fa fa-pencil-square'></i>&nbsp;处理</button >&nbsp;";
				}
				return op;
			}
		});
	...
	function showTable(columns) {
		...
		$("#tableList").bootstrapTable({
			url : jsBasePath + '/customer/mailFox/query.html', //请求后台的URL（*）
			//method: 'get',      //请求方式（*）
			method : 'post',
	...
	function edit(id){
		layer.open({
			   ...
			   offset : ['10%'],
			   area: ['80%', '80%'],
			   content: jsBasePath+'/customer/mailFox/beforEdit.html?id='+id
			}); 
	}
