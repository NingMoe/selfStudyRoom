...
function initTable() {
	...
	$("#menuTable").bootstrapTable({
		url : jsBasePath + '/basic/wxTeacher/query.html', //请求后台的URL（*）
		//method: 'get',      //请求方式（*）
		method : 'post',
				...
				var re="";				
					...
					if(row.status=="1"){
						re += "<button  class='layui-btn layui-btn-mini' style='margin-right:10px;' onclick='return del(\"" 
							+ row.id + "\",2);'><i class='fa fa-pencil-square-o'></i>&nbsp;禁用</button>";
					}
					if(row.status=="2"){
						re += "<button  class='layui-btn layui-btn-mini' style='margin-right:10px;' onclick='return del(\"" 
							+ row.id + "\",1);'><i class='fa fa-pencil-square-o'></i>&nbsp;启用</button>";
					}
					if(row.filter=="2"){
					...
					if(row.filter=="3"){
						re += "<button  class='layui-btn layui-btn-mini' style='margin-right:10px;' onclick='return grantUser(\"" 
							+ row.id + "\",\""+row.name+"\");'><i class='fa fa-pencil-square-o'></i>&nbsp;分配用户</button>";
					}
...
//添加
function add(){
	 var url = jsBasePath+"/basic/wxTeacher/toAdd.html";
	 layer.open({
	     type: 2,
	     shade : [ 0.5, '#000' ],
	     title: "新增菜单", //
...
//编辑
function edit(id){
	 var url = jsBasePath+"/basic/wxTeacher/toEdit.html?id="+id;
	 layer.open({
	     type: 2,
	     shade : [ 0.5, '#000' ],
	     title: "编辑菜单", //
...
function grantDept(id){
	 var url = jsBasePath+"/basic/wxTeacher/toGrantDept.html?menuId="+id;
	 layer.open({
	     type: 2,
	     shade : [ 0.5, '#000' ],
	     title: "分配部门", //
...
//分配用户
function grantUser(id,name){
	 var url = jsBasePath+"/basic/wxTeacher/toGrantUser.html?menuId="+id+"&menuName="+name;
	 layer.open({
	     type: 2,
	     shade : [ 0.5, '#000' ],
	     title: "分配用户", //
...
function del(id,status){
	$.ajax({
		...
		dataType:"json",
		url:jsBasePath+"/basic/wxTeacher/edit.html",
