...
<head>
	...
	<input type="hidden" id="positiveId" name="positiveId" value="${positiveId }">
	...
	<div style="float:left;width:18%;position:fixed;top:0;left:0">
		<ul class="" style="background-color:#fff;margin-left:60%;">
		  ...
		  <li class="nav"><a href="#interviewRecord">面试记录</a></li>
		  <li class="nav"><a href="#positiveMt">转正面谈</a></li>
		</ul>
	</div>
 ...
 -->			</div>
	...
	<div class="layui-collapse" lay-accordion="" style="float:left;width:65%;margin-left:20%" id="interviewRecord">
		 <div class="layui-colla-item"> 
			<h2 class="layui-colla-title">面试记录</h2>
			<div class="layui-colla-content layui-show" id="gtNR">
			
			</div>
		 </div>
	   </div>
	   
	<div class="layui-collapse" lay-accordion="" style="float:left;width:65%;margin-left:20%" id="positiveMt">
		 <div class="layui-colla-item" id="certifColla"> 
			<h2 class="layui-colla-title">转正面谈</h2>
			<div class="layui-colla-content layui-show">
				<div class="layui-form-item" style="width:75%; ">
					<label class="layui-form-label" style="width:14%;">转正面谈记录</label>
					<div class="layui-input-inline" style="width:50%;">
						<textarea id="interviewComment" name="interviewComment" class="layui-textarea">${interviewRecord.interviewComment }</textarea>
					</div>
				</div>
			
				<div class="layui-form-item" style="width:75%; ">
					<label class="layui-form-label" style="width:14%;">面试是否通过</label>
					<div class="layui-input-inline" style="width:50%;">
						<input type="radio" name="interviewResult" title="通过" lay-filter="refuse"
						<c:if test="${interviewRecord.interviewResult eq '1' }">checked="checked"</c:if> 
						 value="1">
				   		<input type="radio" name="interviewResult" title="不通过" lay-filter="refuse" 
				   		<c:if test="${interviewRecord.interviewResult eq '2' }">checked="checked"</c:if> 
				   		value="2">
					</div>
				</div>
				<div class="layui-form-item" id="refuse" style="width:75%;display:none;">		
				<label class="layui-form-label" style="width:14%;">不通过原因</label>
					<div class="layui-input-inline" style="width:50%;">
						<input type="text" id="refuseReason" name="refuseReason" value="${interviewRecord.refuseReason }" class="layui-input">
					</div>
				</div>
				<div class="layui-form-item" style="width:75%; ">
	...
	layui.use(['element','form', 'laydate','upload'], function(){
		...
		$.post(jsBasePath+"/basic/areaInfo/getArea.html",{areaLevel:2,areaCode:nativeCity},function(data,status){
		...
		$.post(jsBasePath + '/recruit/resumeManager/gtRecord.html', {resumeId:844}, function(data, status) {
			if (!data.flag) {
				$("#gtNR").html("沟通记录获取失败");
			}else {
					...
					$.each(data.ac,function(i,item){
						for(var re=0;re<item.actComment.length;re++){
							if(re==0||item.actComment[re].nodeName!=item.actComment[re-1].nodeName){
								html+= '<blockquote class="layui-elem-quote" style="padding:10px;background-color:#c2efcf;border:1px solid #c2efcf;"><span>'+item.companyName+'_'+item.positionName+'_'+item.actComment[re].nodeName+'  ('+item.nodeStartTime+')</span></blockquote>';
							}else{
								html+= '<fieldset class="layui-elem-field layui-field-title"></fieldset>';
							}
			                html+= '<div class="layui-form-item" style="width:75%; ">';
			                html+= '<label class="layui-form-label" style="width:14%;">面试官</label>';
			                html+= '<label class="layui-form-label" style="width:70%;text-align:left;">'+item.actComment[re].name+'</label>';
			                html+= '</div>';	
			                
			                
			                html+= '<div class="layui-form-item" style="width:75%; ">';
			                html+= '<label class="layui-form-label" style="width:14%;">反馈时间</label>';
			                html+= '<label class="layui-form-label" style="width:70%;text-align:left;">'+item.actComment[re].approveTime+'</label>';
			                html+= '</div>';	
			                
			                html+= '<div class="layui-form-item" style="width:75%; ">';
			                html+= '<label class="layui-form-label" style="width:14%;">面试记录</label>';
			                html+= '<label class="layui-form-label" style="width:70%;text-align:left;">'+item.actComment[re].comment+'</label>';
			                html+= '</div>';
			                
			                html+= '<div class="layui-form-item" style="width:75%; ">';
			                html+= '<label class="layui-form-label" style="width:14%;">面试结论</label>';
			                html+= '<label class="layui-form-label" style="width:70%;text-align:left;">';
			                if(item.actComment[re].result==1){
								html+="同意";
							}
							if(item.actComment[re].result==0){
								html+="不同意     (不同意原因："+item.actComment[re].backReason+")";
							}
							if(item.actComment[re].result==2){
								html+="退回     (退回原因："+item.actComment[re].backReason+")";
							}
							if(item.actComment[re].result==3){
								html+="直接通过    (直接通过原因："+item.actComment[re].backReason+")";
							}
							html += '</label>';
			                html+= '</div>';
			                if(item.actComment[re].actScore.length){
			                	html+= '<div class="layui-form-item" style="width:75%; ">';
				                html+= '<label class="layui-form-label" style="width:14%;">面试打分</label>';
				                html+= '<label class="layui-form-label" style="width:70%;text-align:left;">';
