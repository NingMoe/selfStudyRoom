...
function initTable() {
	...
	$("#ccrTable").bootstrapTable({
		url : jsBasePath + '/basic/jzbGrade/query.html', //请求后台的URL（*）
		//method: 'get',      //请求方式（*）
		method : 'post',
		...
		columns : [ {
			...
			formatter : function(value, row, index) {
				var re="";				
					...
					re += "<button  class='layui-btn layui-btn-mini' style='margin-right:10px;' onclick='return configSubject(\"" 
						+ row.id + "\");'><i class='fa fa-pencil-square-o'></i>&nbsp;配置科目</button>";	
...
function queryParams(params) {
	return {
		...
		deptId : $("#deptId").val(),
		gradeName : $.trim($("#gradeName").val())
	};
}
//添加
function add(){
	 var url = jsBasePath+"/basic/jzbGrade/toAdd.html";
	 layer.open({
	     type: 2,
	     shade : [ 0.5, '#000' ],
	     title: "新增年级数据", //
...
//编辑
function edit(id){
	 var url = jsBasePath+"/basic/jzbGrade/toEdit.html?id="+id;
	 layer.open({
	     type: 2,
	     shade : [ 0.5, '#000' ],
	     title: "编辑年级数据", //
...
//删除
function del(id){
	layer.confirm("确定删除该年级数据么?删除后不可恢复!", {
		  btn: ['是','否'] ,//按钮
		  offset: '10%',
		  btnAlign:'c'
		}, function(index){
			$.post(jsBasePath+"/basic/jzbGrade/delete.html",{deleteIds:id},function(data,status){
				layer.close(index);
				if(data!=null){
...
//配置科目
function configSubject(id){
	var url = jsBasePath+"/basic/jzbGradeSubject/index.html?gradeId="+id;
	 layer.open({
	     type: 2,
	     shade : [ 0.5, '#000' ],
	     title: "配置科目", //
