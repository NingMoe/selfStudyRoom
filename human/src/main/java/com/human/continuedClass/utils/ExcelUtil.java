...
public class ExcelUtil<T> implements Serializable {
    ...
    private static final long serialVersionUID = 3031953824540101207L;
    
    /** 时间格式数组（年月日 时分秒） */
    public static final String DATE_FULL_STR = "yyyy-MM-dd HH:mm:ss";
    
    public static final SimpleDateFormat sdf=new SimpleDateFormat(DATE_FULL_STR);
     ...
     *            java输出流 
     */  
    public void getListToExcel(List<T> list, String sheetName, OutputStream output) {  
        try {  
            // 得到所有定义字段  
            Field[] allFields = clazz.getDeclaredFields();  
            ...
            // 每页sheet中数据行数
            int sheetNo=0,sheetSize = 10000; 
            int m=listSize%sheetSize;    
            if(m>0){    
                sheetNo=listSize/sheetSize+1;    
            }else{    
                sheetNo=listSize/sheetSize;    
            }     
            for (int index = 0; index <sheetNo; index++){  
                ...
                for (int i = 0; i < fields.size(); i++) {  
                    ...
                    if (attr.isMark()) {  
                        ...
                        newCellStyle.setVerticalAlignment(HSSFCellStyle.VERTICAL_CENTER);//垂直居中
                        newCellStyle.setAlignment(HSSFCellStyle.ALIGN_CENTER);//水平居中
                    ...
                    sheet.setColumnWidth(i, (int) ((attr.name().getBytes().length <= 4 ? 6 : attr.name().getBytes().length) * 1.5 * 500));  
                    // 设置列中写入内容为String类型  
