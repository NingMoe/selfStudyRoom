...
<body >
	...
	<fieldset class="layui-elem-field" style="padding: 15px;" id="serachFrom">
		...
		<div class="y-role">
            ...
            <div id="toolbar">
            <sec:authorize ifAnyGranted='ROLE_add_msg_temp'>
            <button onClick="add();" type="button"
				class="layui-btn"><li class="fa fa-plus"></li>
				&nbsp;新增模版
			</button></sec:authorize>
			<sec:authorize ifAnyGranted='ROLE_del_msg_temp'><button  class='layui-btn layui-btn-danger' onclick='return bath_delTem();'><i class='fa fa-minus-square'></i>&nbsp;批量禁用</button ></sec:authorize>
            </div>
        </div>
    </div>
    <script type="text/javascript" src="<%=basePath %>/static/basic/js/sms/sms_list.js"></script>
    <script type="text/javascript">
    	function initTable() {
    		//初始化Table 不 
    		$('#tableList').bootstrapTable('destroy');
    		$("#tableList").bootstrapTable({
    			url : jsBasePath + '/basic/sms/queryTem.html', //请求后台的URL（*）
    			//method: 'get',      //请求方式（*）
    			method : 'post',
    			...
    			columns : [{
    				...
    				formatter : function(value, row, index) {
    					var op = "";
    					if (row.state == 0) {
    					    op += "<sec:authorize ifAnyGranted='ROLE_edit_msg_temp'><button  class='layui-btn layui-btn-mini' onclick='return toEdit(\"" + row.id + "\");'><i class='fa fa-pencil-square'></i>&nbsp;编辑</button >&nbsp; </sec:authorize>";
    						op += "<sec:authorize ifAnyGranted='ROLE_del_msg_temp'><button  class='layui-btn layui-btn-danger layui-btn-mini' onclick='return delTem(\"" + row.id + "\",1);'><i class='fa fa-minus-square'></i>&nbsp;禁用</button >&nbsp; </sec:authorize>";
    					}
    					return op;
    				}
    			},{
