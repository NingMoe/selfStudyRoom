...
<input id="userName" type="hidden" value="${userName}">
		...
		<c:forEach var="detail" items="${fb.fbdetail}">
			<div >
				...
				<div>${detail.desc}</div>
					...
					<div class="time" style="margin-top: 3px;text-align:right;">${detail.operUser}回复于<fmt:formatDate value='${detail.operTime}' pattern='yyyy-MM-dd HH:mm:ss'/></div>
						</c:if>
				<c:if test="${detail.type!=1}">
						<div class="time" style="margin-top: 3px;">反馈于<fmt:formatDate value='${detail.operTime}' pattern='yyyy-MM-dd HH:mm:ss'/></div>
				</c:if>
					</div>
		</div>
	...
	<form action="" id="subForm1" class="layui-form"  >
		...
		<label for="desc" class="col-sm-2 control-label layerPadding" ><font style="color: red">*</font>您的建议:</label>
	...
	<textarea class="layui-textarea" rows="5" id="desc1" placeholder="请输入回复内容" lay-verify="required"></textarea>
		...
		<div class="form-group" style="text-align: center; "><button  lay-submit="" lay-filter="sub1"  type="button" class="layui-btn layui-btn-primary">继续反馈</button><%-- &nbsp;&nbsp;<button type="button" class="layui-btn" onclick="closeFeedback('${fb.id}');">&nbsp;关闭问题</button> --%></div>
		</form></div>
		</c:if>
	<c:if test="${fb.state==2}">
<!-- 		<div class="form-group" style="text-align: center; margin-top: 20px;"><button onclick="javascript:window.close();" type="button" class="layui-btn layui-btn-primary "><li class="fa fa-times"></li>&nbsp;关&nbsp;闭&nbsp;</button></div>
 -->	</c:if>
	</div>
...
function showImgs(obj,k){
	...
	function closeFeedback(baseId) {
		layer.open({
			  ...
			  type: 2,
			  area: ['90%', '60%'], //宽高
			  content:  jsBasePath+'/free/feedBack/closeFeedBackView.html?id='+baseId+'&closeType=0&userName='+$("#userName").val()
			});
	}
	
	function addFeedback(baseId){
		$("#addfeedbackDiv").show();
	}
	...
	function jxfeedback(){
		...
		for(var j=0;j<imgs.length;j++){
	      ...
	      xhr.open('post', jsBasePath+'/free/feedBack/jxfeedback.html');
