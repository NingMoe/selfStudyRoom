...
function fdfeedback(){
	...
	$.post(jsBasePath + '/wechat/binding/feedback/fbFeedBack.html', {baseId:$("#baseId").val(),desc:$.trim($("#desc1").val()),operUser:$("#userName").val()}, function(data, status) {
		   if(data.flag){
...
function showPopup(id){
	window.location.href=jsBasePath+'/wechat/binding/feedback/feedBackOperDetail.html?id='+id	
}
	...
	layui.use([ 'element', 'form','flow' ], function() {
		...
		element.on('tab(tabChange)', function(data) {
			if (data.index == 0) {
				...
				$.post(jsBasePath + '/wechat/binding/feedback/getMyOperFeedBack.html', {}, function(res, status) {
					layer.close(index);
					var html = "";
			...
			if (data.index == 1) {
					...
					f2.load({
						elem : '#myOperFeedBackEd', //指定列表容器
							...
							var lis = [];
							//以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
							$.post(jsBasePath + '/wechat/binding/feedback/getMyOperFeedBackEd.html', {
								pageNow : page,pageSize:5
							}, function(res, status) {
								layer.close(index);
								$.each(res.records, function(i, item) {
									var html = "";
