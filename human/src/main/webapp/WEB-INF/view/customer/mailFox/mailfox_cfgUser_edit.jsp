...
<body >
    ...
    function add(){
    	  layer.prompt({title: '输入需要新增的配置人员帐号，并确认', formType: 3}, function(text, index){
        	  ...
        	  $.post(jsBasePath+"/customer/mailFox/addConfig.html",{deptId:$("#deptId").val(),userName:text},function(data,status){
  				layer.close(index); 
  				if(data.flag==false){
    ...
    function removeConfig(userName){
    	...
    	$.post(jsBasePath+"/customer/mailFox/removeConfig.html",{deptId:$("#deptId").val(),userName:userName},function(data,status){
				layer.close(index); 
				if(data.flag==false){
					layer.alert(data.msg,{icon:2});
				}else{
