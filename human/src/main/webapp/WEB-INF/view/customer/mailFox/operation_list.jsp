...
function initTable(){
	 var columns = [{
			...
			formatter : function(value, row, index) {
					...
					if(value==2){
						return "<font color=\"#FFCE42\">待解决</font>";
					}
					if(value==3){
		 ...
		 columns.push({
			...
			formatter : function(value, row, index) {
				var op = "";
				if(row.state==2||row.state==4||row.state==1||row.state==3){
	...
	function showTable(columns) {
		...
		$("#tableList").bootstrapTable({
			url : jsBasePath + '/customer/mailFox/queryMyOper.html', //请求后台的URL（*）
			//method: 'get',      //请求方式（*）
			method : 'post',
	...
	function add(){
		layer.open({
			   ...
			   offset : ['10%'],
			   area: ['80%', '80%'],
			   content: jsBasePath+'/customer/mailFox/add.html'
			}); 
	}
	...
	function edit(id){
		layer.open({
			   ...
			   offset : ['10%'],
			   area: ['80%', '80%'],
			   content: jsBasePath+'/customer/mailFox/edit.html?id='+id
			}); 
	}
