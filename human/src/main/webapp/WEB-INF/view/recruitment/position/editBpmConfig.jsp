...
<head>
	...
	<body style="padding:20px;">
			<form class="layui-form">
				<input type="hidden" id="positionProcessId" name="positionProcessId" value="${positionProcess.id }">
                	...
                	<div class="layui-colla-item">
                		...
                		<div class="layui-colla-content">
                			<div class="layui-form-item">
                				...
                				<input type="text" id="customNodeName" name="customNodeName" lay-verify="required" value="${node.positionNodeConfig.customNodeName }" class="layui-input">
                			</div>
                		</div>
    					...
    					<div class="layui-form-item msgsearch" style="display:none;">
    			 		...
    			 		<div class="layui-form-item">
							...
							<div class="layui-input-inline">
								<input type="radio" name="signType${node.nodeName }" <c:if test="${node.positionNodeConfig.signType eq '0' }">checked="checked"</c:if> title="绝对数" lay-filter="signType" value="0">
								<input type="radio" name="signType${node.nodeName }" <c:if test="${node.positionNodeConfig.signType eq '1' }">checked="checked"</c:if> title="百分比" lay-filter="signType" value="1">
    						</div>
    						<label class="layui-form-label typeLabel">至少同意人数</label>
							<div class="layui-input-inline">
								<input type="text" id="signQuantity" name="signQuantity" value="${node.positionNodeConfig.signQuantity }" class="layui-input">
    						</div>
            			...
            			<div class="layui-form-item">
								...
								<div class="layui-input-inline">
									<input type="radio" name="nopassSignType${node.nodeName }" <c:if test="${node.positionNodeConfig.nopassSignType eq '0' }">checked="checked"</c:if> title="绝对数" lay-filter="nopassSignType" value="0">
									<input type="radio" name="nopassSignType${node.nodeName }" <c:if test="${node.positionNodeConfig.nopassSignType eq '1' }">checked="checked"</c:if> title="百分比" lay-filter="nopassSignType" value="1">
		    					</div>
		    					<label class="layui-form-label typeLabel">至少不同意人数</label>
								<div class="layui-input-inline">
									<input type="text" id="nopassSignQuantity" name="nopassSignQuantity" value="${node.positionNodeConfig.nopassSignQuantity }" class="layui-input">
		    					</div>
		    					<div class="layui-form-mid layui-word-aux">绝对数时请填写1,2... 百分比时请填入50,60...</div>
		            		</div>
            			...
            			<%-- <div class="layui-form-item">
							...
							<div class="layui-input-inline">
								<input type="checkbox" name="notify1" value="1" <c:if test="${node.positionNodeConfig.notify1 eq '1' }">checked="checked"</c:if> title="开启" lay-filter="notify1">
    						</div>
    						<label id="typeLabel" class="layui-form-label notify1" 
    						<c:if test="${empty node.positionNodeConfig.notify1Quan}">style="display:none;"</c:if>
    						>超时时间</label>
							<div class="layui-input-inline notify1" 
								<c:if test="${empty node.positionNodeConfig.notify1Quan}">style="display:none;"</c:if>
								>
								<input type="text" name="notify1Quan" placeholder="单位为分钟" value="${node.positionNodeConfig.notify1Quan }" lay-verify="required" class="layui-input">
    						</div>
            			</div>
            			...
            			<div class="layui-form-item">
							...
							<div class="layui-input-inline">
								<input type="checkbox" name="notify2" value="1" <c:if test="${node.positionNodeConfig.notify2 eq '1' }">checked="checked"</c:if> title="开启" lay-filter="notify2">
    						</div>
    						<label id="typeLabel" class="layui-form-label notify2" 
    						<c:if test="${empty node.positionNodeConfig.notify2Quan}">style="display:none;"</c:if>
    						>超时时间</label>
							<div class="layui-input-inline notify2" 
							<c:if test="${empty node.positionNodeConfig.notify2Quan}">style="display:none;"</c:if>
							>
							<input type="text" name="notify2Quan" placeholder="单位为分钟" value="${node.positionNodeConfig.notify1Quan }" class="layui-input">
    						</div>
            			</div> --%>
            	 		...
            	 		<div class="layui-form-item">
					    	<label class="layui-form-label">视频任务包</label>
					   		<div class="layui-input-inline">
					    		<input type="text" name="videoTask" lay-verify="required" class="layui-input" value="${node.positionNodeConfig.videoTask }">
					    	</div>
				    	</div>
             			...
             			<div class="layui-form-item layui-form-text" style="margin-top:20px;">
					    	...
					    	<div class="layui-input-block" style="width:80%;">
					      	...
					      	${node.positionNodeConfig.textTask }
					      	</textarea>
					    	</div>
				    	</div>
				    	...
				    	<div class="layui-form-item">
							...
							<input type="hidden" name="pic" value="${node.positionNodeConfig.pictureTask }">
							...
							<button id="file${status.index }Button" type="button" 
							<c:if test="${empty node.positionNodeConfig.pictureTask }">style="display:none;"</c:if>
							class="layui-btn layui-btn-small delPic"><i class="layui-icon"></i></button>
							<img width=255px height=200px id="file${status.index }Img" 
							<c:if test="${empty node.positionNodeConfig.pictureTask }">style="display:none;"</c:if>
							<c:if test="${!empty node.positionNodeConfig.pictureTask }">src="${fileurl }${node.positionNodeConfig.pictureTask }"</c:if>
							/>
							</div>
						</div>
						...
						<div class="layui-form-item">
							...
							<input type="checkbox" item-search="${node.nodeId }${item.name }" name="scoreItem" value="${item.name }" title="${item.name }" lay-filter="score">
							</div>
							</c:forEach>
							</div>
            			</div>
		...
		function initScoreItems(scoreItems){
			if(!scoreItems){
				return;
			}
			var scoreItems = JSON.parse(scoreItems);
			 $(scoreItems).each(function(){
				var item = this;
				$("input[type='checkbox'][name='scoreItem']").each(function(){
					if($(this).attr("item-search") == item.nodeId+""+item.scoreItem){
						this.checked = "checked";
						$(this).next().after('<div class="layui-input-inline clearfix" style="width:45px;"><input type="text" value="5" name="scoreItemValue" value="'+item.itemValue+'" placeholder="分值" class="layui-input" style="width:45px;"></div>');
					}
				});
			}); 
		} 
		...
		layui.use(['element','form', 'layedit','upload'], function(){
			...
			form.on('checkbox(score)', function(data){
				if(this.checked){
					$(this).next().after('<div class="layui-input-inline clearfix" style="width:45px;"><input type="text" name="scoreItemValue" placeholder="分值" class="layui-input" style="width:45px;"></div>');
				}else{
			...
			var s = layui.upload({
				url: jsBasePath+'/recruit/position/editProcessConfig.html',
				isAuto:false,
			...
			var getData = function(){
				var data = {};
				var positionProcessId = $("#positionProcessId").val();
				data.id = positionProcessId;
				var positionId = $("#positionId").val();
				var type = $("#type").val();
				...
				$(".processConfig").each(function(index,e){
					...
					nodeConfig.positionProcessId = positionProcessId;
					...
					$(this).find("input[type='checkbox'][name='scoreItem']:checked").each(function(){
						...
						scoreItem.positionProcessId = positionProcessId;
