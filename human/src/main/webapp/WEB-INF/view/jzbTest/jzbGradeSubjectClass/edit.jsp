			...
			<form class="layui-form" id="addForm" action="" method="post" >
			 <input type="hidden" name="id"  value="${jzbGradeSubjectClass.id}">
				...
				<label class="layui-form-label" style="width: 150px;">班号开头相似:</label>
					<div class="layui-input-inline">
						<input type="text" id="containClassNumber" name="containClassNumber" autocomplete="off"  placeholder="班号开头相似,多个以逗号隔开" class="layui-input" value="${jzbGradeSubjectClass.containClassNumber}">
					</div>
				</div>
				...
				<label class="layui-form-label" style="width: 150px;">班号开头排除:</label>
					<div class="layui-input-inline">
						<input type="text" id="noContainClassNumber" name="noContainClassNumber" autocomplete="off"  placeholder="班号开头排除,多个以逗号隔开" class="layui-input" value="${jzbGradeSubjectClass.noContainClassNumber}">
					</div>
				</div>
				...
				<label class="layui-form-label" style="width: 150px;">班级名称关键字（包含）:</label>
					<div class="layui-input-inline">
						<input type="text" id="containClassName" name="containClassName" autocomplete="off"  placeholder="班级名称包含关键字,多个以英文隔开" class="layui-input" value="${jzbGradeSubjectClass.containClassName}">
					</div>
				</div>
				...
				<label class="layui-form-label" style="width: 150px;">班级名称关键字（不包含）:</label>
					<div class="layui-input-inline">
						<input type="text" id="noContainClassName" name="noContainClassName" autocomplete="off"  placeholder="班级名称排除关键字,多个以逗号隔开" class="layui-input" value="${jzbGradeSubjectClass.noContainClassName}">
					</div>
				</div>
					...
					<div class="layui-input-inline">
						<select name="isPass" id="isPass" lay-verify="required">
							...
							<c:forEach items="${levels }" var="level">
					        	<option value="${level.dataValue }" <c:if test="${jzbGradeSubjectClass.isPass eq level.dataValue}">selected="selected"</c:if>>${level.name }</option>
					        </c:forEach>
	    			    </select>
					</div>
				</div>	
