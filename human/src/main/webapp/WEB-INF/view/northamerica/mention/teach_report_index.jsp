	...
	<fieldset class="layui-elem-field" style="padding: 15px;" id="serachFrom">
		...
		<div class="y-role">
            <table class="tableList"  id="tableList">
             ...
             <div id="toolbar">
              <sec:authorize ifAnyGranted='ROLE_men_teachreport_export'>
    ...
    <script type="text/javascript">
    	...
    	function initTable() {
    		...
    		var columns = [{
    			...
    			field : 'mi.studentCode',
    			title : '学员号',
    			align : 'center'
    		}, {
    			field : 'mi.studentName',
    			title : '学员名',
    			align : 'center'
    		}, {
    			field : 'mi.examDate',
    			...
    			field : 'mi.managerTeach',
    			title : '学管师',
    			align : 'center'
    		},{
    			field : 'mi.preExamDate',
    			title : '上次考试日期',
    			align : 'center'
    		},{
    			field : 'mi.isCollege',
    			...
    			formatter : function(value, row, index) {
    				var s="";
    				$.each(row.mi.classList,function(i,classRoom){
    					if(classRoom.type==0){
				...
				if (type == 1 || type == "") {
						columns.push(
											...
											},
											{
												...
												formatter : function(value,
														row, index) {
													var to = row.mi.totalScore
															- row.mi.preTotalScore;
													var re = "";
													if (to > 0) {
							...
							columns.push(
									{
										...
										align : 'center'
							    			...
							    			align : 'center',
							    			formatter : function(value, row, index) {
							    	    		var to =  row.mi.totalScore -  row.mi.preTotalScore;
							    	    		var re="";
							    	    		if (to > 0) {
							    	    			re = "<font style='color:green'>" + row.mi.preTotalScore + "/" +row.mi.totalScore +"</font>";
							    	    		}
							    	    		if (to < 0) {
							    	    			re = "<font style='color: red;''>" +  row.mi.preTotalScore + "/" + row.mi.totalScore +"</font>";
							    	    		}
							    	    		if(to==0){
							    	    			re = row.mi.preTotalScore + "/" + row.mi.totalScore;
							    	    		}
							    	    		return re;
							    			}
							    		},{
										...
										align : 'center',
										formatter : function(value, row, index) {
												return formatMention(row.mi.readScore,row.mi.preReadScore,null)
										}
									},{
									field : 'mi.grammarScore',
									title : '文法<br>上次/本次',
									align : 'center',
									formatter : function(value, row, index) {
											return formatMention(row.mi.grammarScore,row.mi.preGrammarScore,null)
									}
								}, {
									field : 'mi.speakScore',
									title : '数学<br>上次/本次',
									align : 'center',
									formatter : function(value, row, index) {
											return formatMention(row.mi.mathScore,row.mi.preMathScore,null)
									}
								}, {
									field : 'mi.reasonScore',
									title : '科推<br>上次/本次',
									align : 'center',
									formatter : function(value, row, index) {
											return formatMention(row.mi.reasonScore,row.mi.preReasonScore,null)
									}
								}, {
									field : 'mi.writeScore',
									title : '写作<br>上次/本次',
									align : 'center',
									formatter : function(value, row, index) {
											return formatMention(row.mi.writeScore,row.mi.preWriteScore,null)
									}
								});
						}  
						  if (type == 3) {
					...
					function showTable(columns) {
						...
						$("#tableList")
								.bootstrapTable(
										{
											url : jsBasePath
													+ '/northamerica/mention/queryMentionTeachReport.html', //请求后台的URL（*）
											//method: 'get',      //请求方式（*）
											method : 'post',
					...
					function export_list() {
						window.location.href = jsBasePath
								+ "/northamerica/mention/exportTeachMentionReport.html?examType="
