...
<div class="form-horizontal" role="form" >
					<div class="form-group">
						<label for="firstname" class="col-sm-2 control-label"><font style="color: red">*</font>请您选择孩子的年级:</label>
						<div class="col-sm-10">
							<select class="form-control" id="grade" lay-verify="required">
					...
					<div class="form-group">
					<label for="name" class="col-sm-2 control-label"><font style="color: red">*</font>此次投诉建议涉及一下哪些方面【多选】:</label> 
					<div class="col-sm-10 ">
					<select class="form-control" id="type" lay-verify="required"  multiple="multiple" lay-search="">
						<option value="">请选择</option>
						<c:forEach var="org" items="${aspectList }">
				...
				<div class="form-group">
					<label for="name" class="col-sm-2 control-label"><font style="color: red">*</font>请您写下本次建议/投诉的具体事宜:</label> 
					<div class="col-sm-10">
...
function showPopup(id){
		window.location.href=jsBasePath+'/wxCustomer/mailFox/getMailById.html?id='+id
}
				...
				var flow = layui.flow;
				 flow.load({
					    ...
					    ,done: function(page, next){ //到达临界点（默认滚动触发），触发下一页
					      ...
					      $.post(jsBasePath + '/wxCustomer/mailFox/getMailList.html', {pageNow:page,pageSize:5}, function(res, status) {
									layer.close(index);
							$.each(res.records,function(i,item){
								var html="";
								html+= "<a href=\"javascript:showPopup("+item.id+");\" class=\"weui-media-box weui-media-box_appmsg\" style=\"padding:5px;\">";
								html+= "<div class=\"weui-media-box__bd\"><div class=\"weui-media-box__title\">";
								if(item.state==0||item.state==1){
									html+="<div class='stateSpan2'><li class=\"fa  fa-clock-o\"></li>&nbsp;待处理</div>";
								}
								if(item.state==2||item.state==3||item.state==4){
									html+="<div class='stateSpan1'><li class=\"fa  fa-commenting-o\"></li>&nbsp;受理中</div>";
								}
								if(item.state==5){
									html+="<div class='stateSpan3'><li class=\"fa fa-check\"></li>&nbsp;已解决</div> ";
								}
								html+="<font  style=\"color:#0cb29b;font-size:13px;float: right;\">【查看详情】</font></div>";
								html+="<p class=\"weui-media-box__desc\">";
								html+=item.desc;
								if(!item.isRead&&item.readType==1){
									html+="<b>...</b>";
  								}
								html+="</p><p class=\"weui-media-box__desc\"  style=\"text-align: right;\"><font  style=\"font-size:13px;color:#999;\">"+item.createTime+"</font></p>";
								html+= "</div>";
								html+= "</a>";
								lis.push(html);
							});
...
//            计算每块瓦片的宽和高
  ...
  function upload() {
	...
	//  formdata.append('dept', $("#dept").val());
	  formdata.append('grade', $.trim($("#grade").val()));
	  formdata.append('desc', $.trim($("#desc").val()));
	  formdata.append('type', $.trim($("#grade").val()));
   // formdata.append('imagefile', blob);
    xhr.open('post', jsBasePath+'/wxCustomer/mailFox/addMail.html');
