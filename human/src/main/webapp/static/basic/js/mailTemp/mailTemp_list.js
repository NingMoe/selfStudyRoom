 ...
 */
function add(){
	 var url = jsBasePath+"/basic/mailTemp/toAdd.html";
	  layer.open({
      type: 2,
      shade : [ 0.5, '#000' ],
	  offset : [ '5%' ],
 ...
 */
function toEdit(id){
	 var url = jsBasePath+"/basic/mailTemp/toEdit.html?id="+id;
	  layer.open({
      type: 2,
      shade : [ 0.5, '#000' ],
	  offset : [ '5%' ],
...
function delTem(id){
	layer.confirm("禁用后无法重新启用,确定禁用该模版?", {
		  ...
		  btnAlign:'c'
			...
			$.post(jsBasePath+"/basic/mailTemp/delTemp.html",{deleteIds:id},function(data,status){
				layer.close(index);
				if(data!=null){
...
function bath_delTem(){
	...
	layer.confirm("禁用后无法重新启用,确定禁用所选模版?", {
		  ...
		  btnAlign:'c'
			...
			$.post(jsBasePath+"/basic/mailTemp/delTemp.html",{deleteIds:ids},function(data,status){
				layer.close(index);
				if(data!=null){
					layer.msg(data.message);
					if(data.flag==true){
