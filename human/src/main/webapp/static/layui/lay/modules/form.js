    ...
    u = function() {
        this.config = {
            verify: {
                ...
                datetime: [/^$|((\d{4}-\d{2}-\d{2}\d{2}:\d{2}:\d{2})|(\d{2}-\d{2}\d{2}:\d{2}:\d{2})|(\d{2}:\d{2}:\d{2}))/, "日期时间格式不正确"],
                identity: [/^$|((^\d{15}$)|(^\d{17}(x|X|\d)$))/, "请输入正确的身份证号"],
                let_num_un:[ /[A-Za-z0-9_]+$/,"只能输入字母数字和下划线"],
                isScore:[ /^$|(^(0|\d{1,4})(\.\d)?)$/,"成绩输入不正确!"]
            }
        }
    };
        ...
        n = {
            select: function() {
                ...
                y = function(e, r, d) {
                    ...
                    if (!r) {
                        ...
                        function() {
                            ...
                            a = t.attr("lay-value"),
                            n = y.attr("lay-filter"),
                            idvs = t.attr("lay-value"),
                            se = t.parent().parent().prev(),
                            idarr = [],
                            txt = t.text(),
                            ischange = true;
                            if(typeof(se.attr("lay-search"))=="string"){
                            	var ids = t.parent().prev().find(".idinput").val();
                            	var _v = t.attr("lay-value"); 
                            	if(ids && ids!="null" ){
                            		idarr = ids.split(",");
                            		if(i.inArray(_v,idarr)>-1){
                            			idarr.splice(i.inArray(_v,idarr), 1);
                            		}else{
                            			idarr.push(_v);
                					}
                            		idvs = idarr.join(",");
                            		var desArr = [];
                            		t.parent().find("dd").removeClass(s);
                            		for(var k=0;k<idarr.length;k++){
                            			var _dd = t.parent().find("dd[lay-value='"+idarr[k]+"']");
                            			desArr.push(_dd.text());
                            			_dd.addClass(s);
                            		}
                            		txt = desArr.join(",");
                            	}else{
                            	...
                            	t.parent().prev().find(".idinput").val(idvs);
                            }
                            
                            ischange = y.val()!=idvs;
                            return ! t.hasClass(c) && (
                            		y.val(idvs).removeClass("layui-form-danger"), 
                            		h.val(txt), 
                            		(typeof(t.parent().parent().prev().attr("lay-search"))=="string"/* && idarr.length>0*/)
                            		||(t.addClass(s).siblings().removeClass(s)),
                            		void layui.event.call(this, l, "select(" + n + ")", {
                            			elem: y[0],
                            			value: idvs,
                            			othis: e
                            		},ischange))
                ...
                d.each(function(n, l) {
                    ...
                    if(v){
                    	for(var key in d){  
                    		selectedTe += selectedTe?","+$(l).find('option[value="'+d[key]+'"]').text():$(l).find('option[value="'+d[key]+'"]').text();  
                          }      
                    }
                    var h = i(['<div class="layui-unselect ' 
                           + t + (u ? " layui-select-disabled": "") + '">', 
                           '<div class="' 
                           + a 
                           + '">'+(v?'<input type="hidden" value="'+d+'" class="idinput">':'')+'<input type="text" placeholder="' 
                           + (l.options[0].innerHTML ? l.options[0].innerHTML: e) 
                           + '" value="' + (d ? (v?selectedTe:f.html()) : "") + '" ' 
                           + (v ? "": "readonly") 
                        ...
                        return layui.each(e,
                        	...
                        	&&*/ ("optgroup" === t.tagName.toLowerCase() 
                        			? i.push("<dt>" + t.label + "</dt>") 
                        					: i.push('<dd lay-value="' + t.value + '" class="' + ((!!d&&d === t.value) ? s: "") + (t.disabled ? " " + c: "") + '">' + t.innerHTML + "</dd>"))
                        }),
                        i.join("")
                    } (r.find("*")) + "</dl>", "</div>"].join(""));
                    o[0] && o.remove(),
