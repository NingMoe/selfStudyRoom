...
function initTable() {
	//初始化Table 不 
	$('#continueclass').bootstrapTable('destroy');
	$("#continueclass").bootstrapTable({
		...
		columns : [ {
			...
			formatter : function(value, row, index) {
				var page = $("#continueclass").bootstrapTable("getPage");
			...
			formatter : function(value, row, index) {
				var re="";				
					re += "<button  class='layui-btn layui-btn-mini' style='margin-right:10px;' onclick='return edit(\"" 
						+ row.id + "\");'><i class='fa fa-pencil-square-o'></i>&nbsp;配置</button>";
