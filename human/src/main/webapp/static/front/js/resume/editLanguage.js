...
function save(){
	...
	$.ajaxFileUpload({
		type : 'post',
		url : jsBasePath+"/front/resume/editLu.html",
		secureuri : false,
		fileElementId : arrId,
		dataType : 'json',
		data : {
...
function deleteById(){
	var rehId=$("#rlId").val();
	var resumeSeekerId=$("#resumeSeekerId").val();
	$.confirm("确认删除该条语言能力?", function() {
		  //点击确认后的回调函数
		var index = layer.load(1, {shade: [0.8, '#393D49']});
		deleteLanguageById(rehId,resumeSeekerId,index);
	  }, function() {
		  //点击取消后的回调函数
	});
}
function deleteLanguageById(rehId,resumeSeekerId,index){
	var resumeId=$("#resumeId").val();
	var positionId=$("#positionId").val();
	$.ajax({
		type : "post",
		dataType : "json",
		url : jsBasePath+"/front/resume/deleteLanguage.html",
		data : {
			id:rehId
		},
