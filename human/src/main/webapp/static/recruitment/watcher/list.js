...
function initTable() {
	...
	$("#resumeTable").bootstrapTable({
		url : jsBasePath + '/recruit/watcher/query.html', //请求后台的URL（*）
		//method: 'get',      //请求方式（*）
		method : 'post',
		...
		columns : [ {
			...
			formatter : function(value, row, index) {
				return "<button  class='layui-btn layui-btn-mini' onclick='return toSendMsg(\"" 
					+ row.telephone + "\",\""+row.resumeSeekerId+"\",\""+ row.id +"\");'><i class='fa fa-pencil-square-o'></i>&nbsp;发送短信</button>";
			}
		}],
...
function exportSelectInterview(){
	...
	window.location.href = jsBasePath+"/recruit/watcher/exportSelect.html?ids="+ids;
}
//导出本页
function exportThisPage(){
	...
	var pageSize=rows.pageSize;
	 $.ajax({
		type:"POST",
		url:jsBasePath+"/recruit/watcher/exportThisPage.html",
		dataType:"json",
		data:{
			pageSize : pageSize,
    		pageNow :  pageNow,
		...
		success:function(data){
			if (data.flag==true) {
				window.location.href = jsBasePath+"/recruit/exportExcel/exportWatcherList.html";
			} else {
...
//导出全部数据
function exportAll(){
	window.location.href = jsBasePath+"/recruit/watcher/exportAll.html";
}
