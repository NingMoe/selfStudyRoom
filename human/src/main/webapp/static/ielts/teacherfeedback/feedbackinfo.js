...
function initTable() {
	...
	$("#processDefTable").bootstrapTable({
		url : jsBasePath + '/ielts/teacherfeedback/query.html?teacher_id=' + teacher_id, // 请求后台的URL（*）
		// method: 'get', //请求方式（*）
		method : 'post',
		...
		columns :
			[
			 ...
			 onEditableSave: function (field, row, oldValue, $el) {
	                $.ajax({
	                    type: "post",
	                    url: jsBasePath + "/ielts/teacherfeedback/updateteacherfeedback.html",
	                    data: row,
	                    dataType: 'JSON',
...
function deletesteachertktinfo() {
	...
	layer.confirm("确认删除选中的行？", {
		  ...
		  btnAlign:'c'
			...
			layer.close(index);
			$.ajax({
				url : jsBasePath + "/ielts/teacherfeedback/deletesteacherfeedback.html",
				type : "POST",
				dataType : "json",
				data : {
					ids : ids,
