//初始化
$(function() {
	var left_integral_time = "";
	var right_integral_time = "";
	var lefttime = "";
	var righttime = "";
	...
	$("#left_integral_time").val(left_integral_time);
	$("#right_integral_time").val(right_integral_time);
	getteacherinfo(left_integral_time, right_integral_time);//获取单个教师积分详情
	buttionclick();//按钮事件
});
...
function getteacherinfo(left_integral_time, right_integral_time){
	...
	$.ajax({
		url : jsBasePath + "/ielts/teacherself/selectteacherselfintegral.html",
		type : "POST",
		dataType : "json",
		data : {
		...
		success : function(date){
			if(date.flag){
				...
				if (teacherinfo.is_teacher_certificate == 1) {
					model1 += 5;
					$("#is_teacher_certificate_div").html("<label class=\"layui-form-label\" style=\"width: 30%\">有</label>");
					$("#is_teacher_certificate_fen").html("<label class=\"layui-form-label\">5分</label>");
			 	}else{
			 		$("#is_teacher_certificate_div").html("<label class=\"layui-form-label\" style=\"width: 30%\">无</label>");
			 		$("#is_teacher_certificate_fen").html("<label class=\"layui-form-label\">0分</label>");
			 	}
				
				//Celta证书
				if (teacherinfo.is_celta == 1) {
					model1 += 10;
					$("#is_celta_div").html("<label class=\"layui-form-label\" style=\"width: 30%\">有</label>");
					$("#is_celta_fen").html("<label class=\"layui-form-label\">10分</label>");
				}else{
					$("#is_celta_div").html("<label class=\"layui-form-label\" style=\"width: 30%\">无</label>");
					$("#is_celta_fen").html("<label class=\"layui-form-label\">0分</label>");
				}
				...
				if(teacherinfo.abb_num != null && teacherinfo.duty_num != null && teacherinfo.abb_num != 0){
