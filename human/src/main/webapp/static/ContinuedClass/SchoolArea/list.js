...
function initTable() {
	...
	$("#ccrTable").bootstrapTable({
		url : jsBasePath + '/basic/school_area/query.html', //请求后台的URL（*）
		//method: 'get',      //请求方式（*）
		method : 'post',
...
//添加
function add(){
	 var url = jsBasePath+"/basic/school_area/toAdd.html";
	 layer.open({
	     type: 2,
	     shade : [ 0.5, '#000' ],
	     title: "新增校区基础数据", //
...
//编辑
function edit(id){
	 var url = jsBasePath+"/basic/school_area/toEdit.html?id="+id;
	 layer.open({
	     type: 2,
	     shade : [ 0.5, '#000' ],
	     title: "编辑校区基础数据", //
...
//删除
function del(id){
	layer.confirm("确定删除该校区么?删除后不可恢复!", {
		  btn: ['是','否'] ,//按钮
		  offset: '10%',
		  btnAlign:'c'
		}, function(index){
			$.post(jsBasePath+"/basic/school_area/delete.html",{deleteIds:id},function(data,status){
				layer.close(index);
				if(data!=null){
