var zTreeObj;
	...
	var setting = {
		...
		async : {
			enable : true,
			url : jsBasePath+"/manager/resource/queryResource.html",
			type : "post"
		},
		check : {
			enable : true,
	...
	function resourceAdd(parentId,parentName,newLevel){
		 var url = jsBasePath+"/manager/resource/addUI.html?parentId="+parentId+"&parentName="+parentName+"&type="+newLevel;
		  layer.open({
            type: 2,
            shade : [ 0.5, '#000' ],
			offset : [ '20px' ],
	...
	function toEdit(treeId, treeNode){
		 var url = jsBasePath+"/manager/resource/toEdit.html?id="+treeNode.id;
		  layer.open({
            type: 2,
            shade : [ 0.5, '#000' ],
			offset : [ '20px' ],
	...
	function deleteResource(treeId, treeNode){
		layer.confirm('确认要删除所选资源?', {
			  btn: ['是','否'] //按钮
				...
				$.post(jsBasePath+"/manager/resource/delete.html",{id:treeNode.id},function(data,status){
					layer.close(index1);
					if(data!=null){
