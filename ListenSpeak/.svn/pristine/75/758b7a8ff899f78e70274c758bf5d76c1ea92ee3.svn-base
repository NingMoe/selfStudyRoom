		...
		<form class="layui-form" id="addForm" action="" method="post">
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-inline" style="width: 30%;">
					<input type="text" id="classCode" name="classCode"  value='${lst.class_code}' readonly="readonly"
						class="layui-input">
				</div>
				<label class="layui-form-label" style="width: 10%;">班级名称:<font color="red">*</font></label>
				<div class="layui-input-inline" style="width: 30%;">
					<input type="text" id="className" name="className"  lay-verify="required" value="${lst.class_name}"
						 class="layui-input">
				</div>
			</div>
			<div class="layui-form-item" >
				...
				<div class="layui-input-inline" style="width: 30%;">
							<select name="grade" id="grade" lay-verify="required" >
								...
								<c:forEach items="${grade }" var="gra" >
									<option value="${gra.dataValue }"  <c:if test="${gra.dataValue  eq lst.grade}">selected="selected"</c:if>>${gra.name }</option>
								</c:forEach>
							</select>
						</div>
...
<!-- 								<option value="">请选择</option> -->
								<c:forEach items="${subject}" var="sub">
									<option value="${sub.dataValue }" <c:if test="${sub.dataValue  eq lst.subject  }">selected="selected"</c:if>>${sub.name }</option>
								</c:forEach>
							</select>
				</div>
			</div>
			...
			<label class="layui-form-label" style="width: 10%;">失效时间:<font color="red">*</font></label>
				<div class="layui-input-inline" style="width: 30%;">
					<input class="layui-input" placeholder="失效时间" id="validTime" value="${fn:substring(lst.valid_time, 0, 19)}"  lay-verify="required"
